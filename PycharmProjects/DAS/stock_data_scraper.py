...
from config import start_d,end_d
import requests
...
async def scrape_stock_code(session, stock_code, start_d, end_d):
    data = []
    for from_date, to_date in date_ranges(start_d, end_d, days=730):
...
async def scrape_all_stock_codes(stock_codes):
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [scrape_stock_code(session, stock_code, start_d, end_d) for stock_code in stock_codes]
        all_results = await asyncio.gather(*tasks)
        for result in all_results:
            all_data.extend(result)
    return all_data

def date_ranges(start_date_, end_date_, days=730):
    start = datetime.strptime(start_date_, "%d.%m.%Y")
    end = datetime.strptime(end_date_, "%d.%m.%Y")
    while start < end:
